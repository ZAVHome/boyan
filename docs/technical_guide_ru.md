...
5. **Санитизация и восстановление FB2 (FB2 Sanitizer):** конвейер автоматического устранения дефектов разметки `fb2.SanitizeFB2Bytes`: удаление UTF-8 BOM, гарантия пространств имён (`xmlns`, `xmlns:xlink`, `xmlns:l`), замена HTML-сущностей (`&nbsp;`, `&mdash;`, `&laquo;` и т.д.) на числовые коды XML. Позволяет строгому браузерному `DOMParser` и внешним читалкам открывать даже поврежденные или неполные оцифровки.
...
- `GET /api/v1/books` — получение списка книг с пагинацией (`page`, `limit`), поиском (`search`), фильтрацией (`genre`, `publisher`, `year`, `language`) и сортировкой (`sort=recent|title|author`).
...
- `GET /api/v1/covers/{id}` — отдача обложки с генерацией на лету при отсутствии (поддерживает `If-None-Match` и ответ `304 Not Modified`).

#### Таксономия

- `GET /api/v1/authors` — список авторов с количеством книг, поддержкой алфавитного фильтра (`letter`) и поиска (`search`).
- `GET /api/v1/authors/{id}` — профиль автора.
- `GET /api/v1/authors/{id}/books` — список книг конкретного автора.
- `GET /api/v1/series` — список серий с поддержкой алфавитного фильтра (`letter`) и поиска (`search`).
- `GET /api/v1/series/{id}` — метаданные серии.
- `GET /api/v1/series/{id}/books` — список книг серии, упорядоченных по порядковому номеру тома (`series_index`).
...
- **Кураторство каталога книг:**
  ...
  - `PUT /api/v1/admin/books/{id}` — обновление метаданных книги (название, оригинальное название, авторы, серии с индексом, жанры, аннотация, издательство, язык, год, ISBN) с синхронизацией SQLite FTS5.
...
- **Хранилище и фоновые задачи (Task Manager):**
  ...
  - `POST /api/v1/admin/tasks/repair-fb2` — запуск фоновой задачи массовой инспекции и санитизации FB2-файлов в библиотеке.
...
2. **Создание каталогов (стандарт FHS 3.0):**
