...
| **16** | [16_linux_storage_fhs_architecture.md](16_linux_storage_fhs_architecture.md) | **Утверждено** | Архитектура размещения данных на Linux (FHS 3.0): четкое разделение служебного состояния (`/var/lib/boyan/`), кэша обложек (`/var/cache/boyan/covers`) и книжного массива (`/srv/books` или `/var/lib/boyan/library`) для предотвращения переполнения диска. |
| **17** | [17_admin_panel_architecture.md](17_admin_panel_architecture.md) | **Утверждено** | Архитектура панели администратора («Админка») комплекса «Боян»: полноэкранный Dedicated Admin Layout, дашборд и мониторинг хоста/SQLite, управление пользователями (CRUD + публичная регистрация), кураторство книг (метаданные, безопасное удаление, пакетные операции), модерация карантина, асинхронный Task Manager сканера и импорта Calibre, управление конфигурацией `config.yaml`. |
