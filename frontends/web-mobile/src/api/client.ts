...
export const api = {
  ...
  async downloadBookBlob(bookId: string, format: string): Promise<Blob> {
    const token = tokenStorage.get()
    const headers: Record<string, string> = {}
    if (token) {
      headers['Authorization'] = `Bearer ${token}`
    }
    const response = await fetch(`/api/v1/books/${bookId}/download/${format}`, { headers })
    if (!response.ok) {
      throw new ApiError(`Failed to download book: ${response.statusText}`, response.status)
    }
